    ...
    if (isset($_POST['photo'])) {
        ...
        if ($atual == 'entrando') {
            ...
            if (is_null($row3)) {
                ...
                $sql_insere = "INSERT INTO pontos (funcionario_id, data, hora_entrada, justificativa) VALUES (?, ?, ?, ?)";
                $stmt = $conn->prepare($sql_insere);
                $stmt->bind_param("isss", $funcionario_id, $hoje_entrar, $horario, $_SESSION['justificativa']);

                if (!$stmt->execute()) {
                    echo "Erro: " . $sql_insere . "<br>" . $conn->error;
                }
            }
        } elseif ($atual == 'saindo') {
            ...
            if ($result->num_rows != 0) {
                ...
                $sql_update = "UPDATE pontos SET hora_saida = ? WHERE funcionario_id = ? AND data = ?";
